...
function App() {
  ...
  const [databaseInfo, setDatabaseInfo] = reactExports.useState();
  ...
  async function writeProbe() {
    setBusy(true);
    try {
      const label = `essai-${(/* @__PURE__ */ new Date()).toISOString()}`;
  ...
  async function printProbe() {
    ...
    try {
      ...
      setStatus({
        ...
        text: result.value.printed ? `Ticket composé (${String(result.value.byteCount)} octets via ${result.value.via})` : `Impression non effectuée : ${result.value.reason ?? "raison inconnue"}`
      });
    } finally {
      setBusy(false);
    }
